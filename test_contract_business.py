...
if __name__ == "__main__":
    ...
    test.setup()  # 执行前置处理
    test.test01_login_success()  # 执行登录测试
    # 如果登录成功，继续执行其他测试
    if test.token:
        test.test02_add_course()  # 执行课程新增测试
        test.test03_upload_contract()  # 执行合同上传测试
        test.test04_add_contract()  # 执行合同新增测试
    test.teardown()  # 执行后置处理